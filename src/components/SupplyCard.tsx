...
export function SupplyCard({ item, own, pending, onClaim }: { item: ActiveBoardItem; own: boolean; pending: boolean; onClaim: () => void }) {
  return <article className="supply-card" aria-label={`${item.item_type} ${quantity(item)} at berth ${item.berth}`}>
    <div className="card-top"><Countdown expiresAt={item.expires_at} />{own && <span className="status status-info">YOUR POST</span>}</div>
    ...
    <button className="button button-primary" disabled={own || pending} onClick={onClaim} aria-label={`Claim ${quantity(item)} ${item.item_type} at Berth ${item.berth}`}>
      {own ? 'Your listing' : pending ? 'Claiming…' : 'Claim'}
    </button>
  </article>
}
